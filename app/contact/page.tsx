...
export const metadata: Metadata = {
  title: "Contact",
  description:
    "Contact Meat Zoo for orders and inquiries through phone or WhatsApp.",
};
    ...
    <section className="bg-brand-cream">
      <div className="mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 lg:px-10">
        ...
        <p className="mt-4 max-w-2xl text-base leading-8 text-brand-muted">
          Reach us directly for product availability, delivery timings, and
          quick support. We keep communication simple and fast.
        </p>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {contactItems.map((item) => {
            ...
            return (
              <div
                key={item.label}
                className="rounded-2xl border border-brand-dark/10 bg-brand-white p-6 shadow-sm"
              >
                ...
                {item.href ? (
                  <a
                    href={item.href}
                    className="mt-2 block text-lg font-semibold text-brand-dark transition hover:text-brand-primary"
                  >
                    {item.value}
                  </a>
                ) : (
        ...
        <div className="mt-8 rounded-2xl border border-brand-dark/10 bg-brand-white p-6 shadow-sm">
          <p className="text-sm leading-7 text-brand-muted">
            Prefer messaging? Tap below to order directly:
          </p>
          <a
