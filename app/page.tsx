...
const heroHighlights = [
  "Freshly prepared cuts every day",
  "Hygienic handling and cold-chain packaging",
  "Direct WhatsApp ordering in one tap",
];

export const metadata: Metadata = {
  title: "All Products",
  description:
    "Explore premium fresh chicken products and order instantly on WhatsApp.",
};
  ...
  return (
    <div className="bg-brand-cream">
      <section className="relative isolate overflow-hidden">
        <Image
          src="/images/hero.svg"
          alt="Fresh chicken delivery hero background"
          fill
          priority
        ...
        <div className="relative mx-auto flex min-h-[78vh] w-full max-w-7xl items-center px-6 py-20 sm:px-8 lg:px-10">
          <div className="max-w-2xl text-brand-white">
            <p className="mb-4 inline-flex rounded-full border border-brand-white/20 bg-brand-white/10 px-4 py-2 text-sm tracking-wide">
              Premium Poultry, Everyday Fresh
            </p>
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-brand-white/85 sm:text-lg">
              Shop quality chicken cuts with a modern, hassle-free flow. Browse
              products, tap order, and confirm instantly on WhatsApp.
            </p>
            <div className="mt-8">
              <a
                href={heroCtaLink}
            ...
            <ul className="mt-10 grid gap-3 text-sm text-brand-white/90 sm:grid-cols-2">
              {heroHighlights.map((item) => (
                <li
                  key={item}
                  className="rounded-xl border border-brand-white/15 bg-brand-white/8 px-4 py-3 backdrop-blur-sm"
                >
                  {item}
                </li>
              ))}
      ...
      >
        <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-3 font-display text-3xl font-semibold text-brand-dark sm:text-4xl">
              Handpicked Chicken Cuts
            </h2>
          </div>
          <p className="max-w-xl text-sm leading-7 text-brand-muted sm:text-base">
            Choose from our ready-to-order bestsellers. Every item below opens
            WhatsApp with a pre-filled order message.
          </p>
        </div>
